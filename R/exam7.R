...
library(jsonlite)
library(httr)
url <- 'https://data.ex.co.kr/openapi/restinfo/restWeatherList?key=4062158448&type=json&sdate=20221006&stdHour=10'
url
res <- GET(url)
...
one<-NULL
try(one<-remDr$findElements(using='css selector', 'p'))
print(one)
print(length(one))
